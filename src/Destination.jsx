...
const Destination = () => {
  const { dstntionNav, loadDestination, currDestination } =
        ...
        <div className="destnDesc">
          <div className="destnNav">
            {dstntionNav.map((name, id) =>
              name !== place.name ? (
                <button key={id} onClick={() => loadDestination(name)}>
                  {name}
                </button>
              ) : (
                <button
                  key={id}
                  onClick={() => loadDestination(name)}
                  style={{ borderBottom: ".3rem solid #FFFFFF" }}
                >
                  {name}
                </button>
              )
          ...
          <div className="destnContent">
            ...
            <div className="info">
              <div className="distance">
                avg. distance<span>{place.distance}</span>
              </div>
              <div className="time">
                est. time travel<span>{place.travel}</span>
              </div>
            </div>
          </div>
        </div>
